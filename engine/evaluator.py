    ...
    def val_func_process_rgbX(self, input_data, input_modal_x, device=None):
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                ...
                #score = F.softmax(score[0], dim=1) + F.softmax(score[1], dim=1)
                score = score[0]
                if self.is_flip:
                    input_data = input_data.flip(-1)
                ...
                score = torch.exp(score)
                #print("=======================================================") 
                #input1 = torch.randn(1, 3, 480, 640).cuda()
                #input2 = torch.randn(1, 3, 480, 640).cuda()
                #flops, params = profile(self.val_func, inputs=(input1, input2))
                #print(flops)
                #print(params)
                #print("model have {} paramerters in total".format(sum(x.numel() for x in self.val_func.parameters())))
                #print("=======================================================")
