...
from ..net_utils import FeatureCalibrationModule as FCM
from ..net_utils import FeatureIntegrationModule as FIM
import math
import time
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512], 
        ...
        self.FCMs = nn.ModuleList([
                    FCM(dim=embed_dims[0], reduction=16, kernel_size=3, sa_type=0),
                    FCM(dim=embed_dims[1], reduction=16, kernel_size=3, sa_type=0),
                    FCM(dim=embed_dims[2], reduction=16, kernel_size=3, sa_type=0),
                    FCM(dim=embed_dims[3], reduction=16, kernel_size=3, sa_type=0)])
        
        self.FIMs = nn.ModuleList([
                    FIM(dim=embed_dims[0], reduction=16, kernel_size=3, padding=1, bias=True, groups=16),
                    FIM(dim=embed_dims[1], reduction=16, kernel_size=3, padding=1, bias=True, groups=16),
                    FIM(dim=embed_dims[2], reduction=16, kernel_size=3, padding=1, bias=True, groups=16),
                    FIM(dim=embed_dims[3], reduction=16, kernel_size=3, padding=1, bias=True, groups=16)])
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        x_rgb, x_e = self.FCMs[0](x_rgb, x_e)
        x_fused = self.FIMs[0](x_rgb, x_e)

        outs_fused.append(x_fused)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb, x_e = self.FCMs[1](x_rgb, x_e)
        x_fused = self.FIMs[1](x_rgb, x_e)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb, x_e = self.FCMs[2](x_rgb, x_e)
        x_fused = self.FIMs[2](x_rgb, x_e)
        ...
        x_rgb = x_rgb.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x_rgb, x_e = self.FCMs[3](x_rgb, x_e)
        x_fused = self.FIMs[3](x_rgb, x_e)
    ...
    def forward(self, x_rgb, x_e):
        outs_fused = self.forward_features(x_rgb, x_e)
        return outs_fused
